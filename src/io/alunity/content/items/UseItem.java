package io.alunity.content.items;

import java.util.List;
import java.util.Optional;

import io.alunity.Server;
import io.alunity.content.PetCollector;
import io.alunity.content.PlatinumTokens;
...
import io.alunity.content.bosses.bryophyta.BryophytaNPC;
...
import io.alunity.content.item.lootable.impl.CrystalChest;
import io.alunity.content.item.lootable.impl.FoeMysteryBox;
import io.alunity.content.items.item_combinations.Godswords;
import io.alunity.content.items.pouch.RunePouch;
import io.alunity.content.lootbag.LootingBag;
import io.alunity.content.minigames.warriors_guild.AnimatedArmour;
...
import io.alunity.content.skills.crafting.*;
import io.alunity.content.skills.firemake.Firemaking;
import io.alunity.content.skills.herblore.*;
...
import io.alunity.content.trails.MasterClue;
...
import io.alunity.util.logging.player.ItemOnItemLog;
	...
	public static void ItemonItem(final Player c, final int itemUsed, final int useWith, final int itemUsedSlot, final int usedWithSlot) {
		...
		if((itemUsed == 24511 && useWith == 24422)) {//harmonised staff
			if (c.getItems().playerHasItem(24511, 1)&& c.getItems().playerHasItem(24422, 1)) {
				...
				c.sendMessage("@red@You successfully add the orb to the staff!");
			} else {
		...
		if((itemUsed == 24268 && useWith == 10828)) {//faceguard
			if (c.getItems().playerHasItem(24268, 1)&& c.getItems().playerHasItem(10828, 1)) {
				...
				c.sendMessage("@red@You successfully make the Neitiznot faceguard!");
			} else {
				c.sendMessage("You do not have all the stuff to create this.");
				return;
			}
